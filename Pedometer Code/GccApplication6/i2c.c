...
void i2c_init(void)
{
    TWSR0 &= ~((1 << TWPS1) | (1 << TWPS0)); // pre-scalar 1
    TWBR0 = ((F_CPU / F_I2C) - 16) / 2; // baud rate factor 12
}
...
{
    int8_t status = 0;
    TWDR0  = byteData; // load data buffer with data to be transmitted
    TWCR0 |= (1 << TWINT); // clear interrupt flag
...
uint8_t i2c_rx_byte(bool response)
{
    ...
    if (response == ACK) {
        TWCR0 |= (1 << TWEA); // generate ACK
    } else {
        TWCR0 &= ~(1 << TWEA); // generate NACK
    }
